...
const defaultProcedures = [
  ...
  {
    title: "Procédure Réclamations",
    description: "Traitement des réclamations et des retours stagiaires",
    icon: BookOpen,
    content: [
      "Réception et enregistrement de la réclamation",
      "Analyse et qualification de la réclamation",
      "Mise en place d'actions correctives",
      "Suivi et clôture de la réclamation",
    ],
  },
];
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="space-y-4">
        {defaultProcedures.map((proc, idx) => {
            ...
            <div key={idx} className="bg-card rounded-xl border border-border p-6">
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  <h2 className="text-lg font-heading font-semibold">{proc.title}</h2>
                  <p className="text-sm text-muted-foreground mt-0.5">{proc.description}</p>

                  {proc.referent && (
                    <div className="flex items-center gap-2 mt-3 text-sm">
                      <User className="w-4 h-4 text-muted-foreground" />
                      <span className="font-medium">Référent :</span>
                      <span>{proc.referent}</span>
                    </div>
                  )}
